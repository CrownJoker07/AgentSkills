---
name: unity-bugfix
description: 诊断 Git 管理的 Unity 项目 BUG，并在项目配置 origin、具备网络和推送权限时完成修复，覆盖 C#、场景、Prefab、资源、Package、编译错误、运行时异常、UI 和数据绑定问题。用户要求分析 Unity BUG 时只读诊断；用户明确要求修复时，要求其提供项目路径，创建独立 worktree，优先使用 OpenCode 修改，由当前 Agent 强制 Review 和测试后提交并推送。输入来自飞书消息、截图或附件时也使用本 Skill。
compatibility: 完整修复流程要求 Bash、Git、网络、origin 远程仓库、推送权限以及在 $HOME/.worktrees 下创建目录的权限；OpenCode 和飞书能力可选。
---
...
任务来自飞书且具有消息上下文时，仅使用可用的飞书能力在同一原话题发送阶段进度和最终结果；不要发送到其他话题或联系人，也不要在 Skill 中假设固定 CLI 命令。发送前检查并脱敏，只发送必要摘要，不发送凭据、完整内部路径、未经处理的日志、无关代码或其他敏感信息。无法确认内容已经安全脱敏时不通过飞书发送，改在当前会话说明阻断原因。其他来源只在当前会话更新。
