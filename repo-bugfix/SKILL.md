...
用户明确提供仓库目录时直接使用。用户只提供仓库名时，使用当前运行环境明确提供的 workspace 根目录，在 `<workspace>/repos` 的直接子目录中，将该名称与目录名或 `origin` 仓库名精确比较；可忽略大小写和末尾的 `.git`。没有唯一匹配或运行环境没有提供 workspace 时，要求用户补充仓库路径。
